...
# ============================================================================
# 用法:
#   ./cc-agent/scripts/run_tensorf_baseline.sh <organ> <views> [GPU]
#
# 器官: foot, chest, head, abdomen, pancreas
# 视角: 3, 6, 9
#
# 示例:
#   ./cc-agent/scripts/run_tensorf_baseline.sh foot 3 0
#   ./cc-agent/scripts/run_tensorf_baseline.sh chest 6 1
# ============================================================================
...
# 输出目录
OUTPUT_DIR="${R2_ROOT}/output/baselines/tensorf"
EXP_NAME="${ORGAN}_${VIEWS}views_tensorf"
mkdir -p "${OUTPUT_DIR}/${EXP_NAME}"

# 生成临时配置文件
CONFIG_FILE="/tmp/tensorf_${ORGAN}_${VIEWS}views_config.yaml"
cat > "$CONFIG_FILE" << EOF
exp:
  expname: ${EXP_NAME}
  expdir: ${OUTPUT_DIR}/

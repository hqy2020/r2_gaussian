#
# FSGS model adapted for CT reconstruction
#
# Based on: FSGS: Real-Time Few-Shot View Synthesis using Gaussian Splatting
# Paper: https://arxiv.org/abs/2312.00451
#
# 核心差异 (相比 X-Gaussian):
# - 新增 confidence 属性用于置信度加权
# - 新增 init_point 属性记录初始点云
# - 新增 proximity() 方法实现 FSGS 核心近邻密化
...
class FSGSModel(GaussianBaseModel):
    """
    FSGS 模型实现 (CT 适配版)

    核心功能:
    - proximity() 方法: FSGS 核心近邻密化
    - confidence 属性: 置信度加权渲染
    - dist_prune() 方法: 基于初始点云距离剪枝

    与 X-Gaussian 的主要差异:
    - 使用 opacity (_opacity) 而非 density (_density)
    - 使用球谐特征 (_features_dc, _features_rest) 表示颜色
    - 新增 proximity 密化和 confidence 加权
    """
    ...
    def __init__(self, config: FSGSConfig = None, scale_bound=None, args=None):
        """
        初始化 FSGS 模型

        Args:
            config: FSGS 配置
            scale_bound: 尺度边界 (与 R2-Gaussian 兼容)
            args: 额外参数
        """
        self.config = config or FSGSConfig()
        ...
        # FSGS 特有属性
        self.confidence = torch.empty(0)  # 置信度加权
        self.init_point = torch.empty(0)  # 记录初始点云 (用于 dist_prune)
    ...
    def create_from_pcd(self, points: np.ndarray, colors: Optional[np.ndarray] = None,
        ...
        # 记录初始点云 (用于 dist_prune)
        self.init_point = fused_point_cloud.clone().detach()
        ...
        print(f"FSGS: Number of points at initialization: {num_points}")
        ...
        # 初始 confidence (全部为 1)
        confidences = torch.ones((num_points, 1), dtype=torch.float, device="cuda")
    ...
    def training_setup(self, opt):
        ...
        l = [
            {'params': [self._xyz],
             'lr': self.config.position_lr_init * self.spatial_lr_scale,
             'name': 'xyz'},
        ...
        self.xyz_scheduler_args = get_expon_lr_func(
            lr_init=self.config.position_lr_init * self.spatial_lr_scale,
    ...
    def restore(self, state: Dict, opt):
        ...
        self.confidence = state.get('confidence', torch.ones_like(state['_opacity']))
        self.init_point = state.get('init_point', state['_xyz'].clone().detach())
    ...
    # ============ FSGS 核心: Proximity 密化 ============

    def proximity(self, scene_extent: float, N: int = None):
        """
        FSGS 核心: Proximity-guided Densification

        在稀疏区域自动补充高斯。选择距离最近邻较远且尺度较大的高斯，
        在它们与邻居之间的中点位置生成新高斯。

        算法 (FSGS Paper Sec. 3.2):
        1. 计算每个高斯到最近邻的距离
        2. 选择条件: dist > 5 * extent AND max_scale > extent
        3. 在选中点与其邻居之间生成新高斯 (中点位置)

        Args:
            scene_extent: 场景范围 (cameras_extent)
            N: 每个源点生成的新点数 (默认使用配置)
        """
        if N is None:
        ...
        scale_threshold = self.config.proximity_scale_multiplier * scene_extent

        selected_pts_mask = torch.logical_and(
            dist > dist_threshold,
            torch.max(self.get_scaling, dim=1).values > scale_threshold
        )
        ...
        print(f"FSGS proximity: added {len(new_xyz)} new Gaussians")

    # ============ 距离剪枝 ============

    def dist_prune(self, threshold: float = None):
        """
        基于到初始点云的距离剪枝

        移除距离初始点云过远的高斯 (可能是漂移的点)

        Args:
            threshold: 距离阈值 (默认使用配置)
        """
        if threshold is None:
            threshold = self.config.dist_prune_threshold

        if self.init_point.shape[0] == 0:
            return  # 无初始点云

        # 计算到初始点云的 Chamfer 距离
        from r2_gaussian.utils.general_utils import chamfer_dist
        dist = chamfer_dist(self.init_point, self._xyz)
        valid_points_mask = (dist < threshold)
        ...
        print(f"FSGS dist_prune: removed {(~valid_points_mask).sum()} Gaussians")
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest,
        ...
        # 新高斯的 confidence 初始化为 1
        self.confidence = torch.cat([
            self.confidence,
            torch.ones((new_opacities.shape[0], 1), device="cuda")
        ], dim=0)
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, iteration=None):
        ...
        # FSGS 核心: proximity 密化 (仅在早期调用)
        if (self.config.enable_proximity and
            iteration is not None and
            iteration < self.config.proximity_until_iter):
